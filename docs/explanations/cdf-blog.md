...
is culmination of several years of effort within [SAS](https://www.sas.com)
DevOps to convert from large ship events to CI/CD. We built the first version
internally to facilitate CI/CD in a complex, aging build system. The result
enables SAS to build, package, scan, promote, and ship thousands of artifacts
daily.
...
artifacts more quickly. The challenge was twofold. First, create a system that
could allow disparate pieces of our pipeline to communicate and chain together.
...
Reality was not so kind. Our source code was (and still is) scattered across
...
message queue. Other services that we call "watchers" watch the message queue
...
When an event has been posted to a receiver, EPR will emit a message to
Redpanda.
...
formatted at the root level, with a free-form JSON `payload` field that is field
that is validated against the schema of its receiver. Each event contains a
Boolean `success`` field that represents whether an action was successful or
not.
...
internally that do a variety of things. One of our more popular watchers’ fires
...
The first big problem we ran into was that the lack of restrictions on who could
use receivers and groups. This meant that anyone could post passing events to
any receiver, which in turn could trigger any associated groups. Add some lazy
...
Role Based Access Control (RBAC) to receivers and groups but ultimately decided
not to in favor of development speed. The tradeoff was some unfortunate hacks
that persist to this day. Now that EPR is open sourced, we intend to implement a
more robust solution soon.
...
management signoff, etc. If you're willing to write the automation, there's no
...
CVE in our codebase within minutes.

### Pipeline Auditing

In the same vein as security scanning, EPR gives us an immutable record of how
an artifact traveled through the pipeline. Failed pipeline tasks are tracked by
...
- [Event Provenance Registry on GitHub](https://github.com/sassoftware/event-provenance-registry)

Announcing the Event Provenance Registry (EPR) Open Source Project

Event Provenance Registry is a culmination of several years of SAS's effort to
...
Each of these fields is just a string, though we strongly recommend you impose
some standards for how each is formatted, depending on your situation. NVRPP is
based off the NEVRA from Fedora. It allows us to represent most types of
...
version. You might name a receiver by the action it represents like
golang-build-complete. Types might be things like build.finished or
artifact.packaged. Receivers may have multiple events that correspond with them.
...
Events are a record of some action that took place in your pipeline and whether
it was successful. Each event contains an NVRPP and is linked to a receiver by
...
"description": "Send an event to release our application if all pipeline tasks
have passed.",
...
developers are no different. We discovered that developers especially don't like
